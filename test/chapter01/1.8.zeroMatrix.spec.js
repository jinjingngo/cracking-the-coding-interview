...
import * as funcs from '../../src/chapter01/1.8.zeroMatrix.js';
const { expect } = chai;

for (let key in funcs) {
  const func = funcs[key];
  describe(`chapter01-1.8 ${key}`, function () {
    
    it('throws an error when null/undefined matrix', function () {
    ...
    it('returns matrix unchanged with 1,1 matrix', function () {
      const matrix = [[0]];
      const expected = [[0]];
      expect(func(matrix)).to.eql(expected);
    });

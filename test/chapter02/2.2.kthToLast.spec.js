...
import * as funcs from '../../src/chapter02/2.2.kthToLast.js';
const { expect } = chai;

for (let key in funcs) {
  const func = funcs[key];

  describe(`chapter02-2.2 ${key}`, function () {
    it('throws an error it list is not an array', function () {
      expect(() => func(null)).to.throw('invalid list');
      expect(() => func(undefined)).to.throw('invalid list');
    });

    it('throws an error if list is short than k', function () {
